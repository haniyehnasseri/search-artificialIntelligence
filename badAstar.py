...
def findHueristic(Patients,newrow,newcol):
	_sum = 0
	for i in range(0,len(Patients[0])):
		_sum += abs(Patients[0][i] - newrow) + abs(Patients[1][i] - newcol)
	return _sum
...
def doAction(action,curNode,PatientsSize):
	...
	if(action == 'L'):
		...
		if(newPatientsLocation == [-1]):
			newHueristic = findHueristic(PatientsLocation,ambulanceLocation_row,ambulanceLocation_col - 1)
			return [[ambulanceLocation_row,ambulanceLocation_col - 1],PatientsLocation,Hospitals,1,newHueristic]
		else:
			newHueristic = findHueristic(newPatientsLocation[0],ambulanceLocation_row,ambulanceLocation_col - 1)
			return [[ambulanceLocation_row,ambulanceLocation_col - 1],newPatientsLocation[0],newPatientsLocation[1],1,newHueristic]
	...
	if(action == 'D'):
		...
		if(newPatientsLocation == [-1]):
			newHueristic = findHueristic(PatientsLocation,ambulanceLocation_row,ambulanceLocation_col - 1)
			return [[ambulanceLocation_row + 1,ambulanceLocation_col],PatientsLocation,Hospitals,1,newHueristic]
		else:
			newHueristic = findHueristic(newPatientsLocation[0],ambulanceLocation_row,ambulanceLocation_col - 1)
			return [[ambulanceLocation_row + 1,ambulanceLocation_col],newPatientsLocation[0],newPatientsLocation[1],1,newHueristic]
...
def getInitialHueristic(hospitals):
	bestSum = 0
	for i in range(0,len(Patients[0])):
		bestSum += abs(Patients[0][i] - Ambulance[0])  + abs(Patients[1][i] - Ambulance[1])
...
###BFS SEARCH###
Astar2Solution = aStarSearch()
finish = datetime.datetime.now()
print(finish - start)
#print(Astar2Solution)